            ...
            {schedule?.batches?.map((batch) => {
              return (
                <button
                  ...
                  key={batch?.id}
                  className={`w-full p-2 flex flex-col flex-wrap justify-start items-start border-2 border-indigo-700 rounded-md shadow-md text-[16px] font-[500] disabled:opacity-50 relative gap-1 ${
                ...
                >
                  ...
                  <div className="px-3 text-left flex flex-col gap-0 w-fit mx-auto">
                    {batch?.schedule?.split(",")?.map((schedule) => (
                      <span
                        key={schedule}
                        className={`block text-left w-fit ${
                          batch?.id === details?.batch
                            ? "text-white"
                            : "text-indigo-900"
                        }`}
